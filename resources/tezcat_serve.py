...
def _install_log_tee():
    log_dir = os.environ.get(
        'TEZCAT_LOG_DIR',
        os.path.join(os.path.expanduser('~'), '.npcsh', 'tezcat', 'logs'),
    )
    try:
...
if __name__ == "__main__":
    ...
    default_port = '7141' if is_dev else '5141'
    port = os.environ.get('TEZCAT_PORT', default_port)
    frontend_port = os.environ.get('FRONTEND_PORT', '7341' if port == '7141' else '6341')
    ...
    start_flask_server(
        ...
        db_path=os.path.expanduser('~/.npcsh/tezcat/tezcat.db'),
